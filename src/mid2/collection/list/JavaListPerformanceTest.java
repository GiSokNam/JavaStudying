...
public class JavaListPerformanceTest {
    public static void main(String[] args) {
        ...
        getIndex(arrayList, loop, size / 2); // 중간에 조회
        getIndex(arrayList, loop, size - 1); // 제일 뒤에 조회

        System.out.println("==ArrayList 검색==");
        search(arrayList, loop, 0); // 제일 앞에 검색
        search(arrayList, loop, size / 2); // 중간에 검색
        search(arrayList, loop, size - 1); // 제일 뒤에 검색
        ...
        getIndex(linkedList, loop, size / 2); // 중간에 조회
        getIndex(linkedList, loop, size - 1); // 제일 뒤에 조회

        System.out.println("==LinkedList 검색==");
        search(linkedList, loop, 0); // 제일 앞에 검색
        search(linkedList, loop, size / 2); // 중간에 검색
        search(linkedList, loop, size - 1); // 제일 뒤에 검색
